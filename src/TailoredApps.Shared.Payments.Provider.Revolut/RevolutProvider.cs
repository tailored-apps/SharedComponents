...
{
    Task<RevolutOrderResponse?> CreateOrderAsync(PaymentRequest request);
    Task<RevolutOrderResponse?> GetOrderAsync(string orderId);
...
public class RevolutServiceCaller : IRevolutServiceCaller
{
    ...
    public async Task<RevolutOrderResponse?> CreateOrderAsync(PaymentRequest request)
    {
        ...
        return JsonSerializer.Deserialize<RevolutOrderResponse>(json);
    }

    public async Task<RevolutOrderResponse?> GetOrderAsync(string orderId)
    {
        using var client = CreateClient();
...
public class RevolutProvider : IPaymentProvider
{
    ...
    {
        var order = await caller.CreateOrderAsync(request);
        return new PaymentResponse
        {
            PaymentUniqueId = order?.Id,
    ...
    {
        var order = await caller.GetOrderAsync(paymentId);
