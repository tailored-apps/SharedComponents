...
public class Przelewy24ServiceCaller : IPrzelewy24ServiceCaller
{
    ...
    private HttpClient CreateClient()
    {
        ...
        var credentials = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{options.PosId}:{options.ApiKey}"));
    ...
    public string ComputeSign(string sessionId, int merchantId, long amount, string currency)
    {
        ...
        var bytes = SHA384.HashData(Encoding.UTF8.GetBytes(json));
    ...
    public bool VerifyNotification(string body)
    {
        try
        {
            ...
            var expected = Convert.ToHexString(SHA384.HashData(Encoding.UTF8.GetBytes(json))).ToLowerInvariant();
